...
`local` 프로파일 기준 — 로컬 머신에서 docker-compose 로 띄운 인프라(PostgreSQL / Redis / Kafka / Zipkin) 에 붙어 동작합니다. GCP 환경(Kafka 3브로커 클러스터, Redis Sentinel HA) 연동은 `dev` 프로파일에서 별도로 처리됩니다.
...
> ⚠️ IntelliJ 인코딩이 UTF-8 인지 반드시 확인. (Settings → Editor → File Encodings → 모두 UTF-8) Windows CP949 로 되어있으면 한글 문자열이 깨지면서 Spotless 가 실패합니다.
...
`.env` 의 기본값(`SPRING_PROFILES_ACTIVE=local`, DB 계정 `goggles/goggles`, Kafka `localhost:9092`)은 docker-compose 기본 설정과 동일하므로 **로컬 실행에서는 그대로 두면 됩니다.**
...
| `goggles-kafka` | 9092 | Kafka KRaft 단일 브로커 (zookeeper 없음) |
| `goggles-kafka-ui` | 8989 | Kafka GUI — http://localhost:8989 |
...
```bash
docker compose ps
# 다섯 컨테이너 모두 STATUS 가 "Up (healthy)" 또는 "Up" 이어야 함
```

> 🕐 Kafka 는 헬스체크 통과까지 30초 정도 걸립니다. `docker compose logs -f kafka` 로 `Kafka Server started` 확인 후 다음 단계로.
...
2. `application.yaml` 의 `spring.profiles.default: local` 설정으로 별도 지정 없이도 로컬 프로파일로 뜸. 명시하고 싶으면 Run Configuration → Active profiles → `local`
...
| Kafka 토픽 | http://localhost:8989 → Topics | (자동 생성, 메시지 수신 시 표시) |

기동 로그에서 보이면 정상:
```
Started LectureServiceApplication in X.XXX seconds
HikariPool-1 - Start completed.
[Kafka] partitions assigned: [order.enrollment.completion-0]
```
...
1. 학생A 수강 목록 조회 → JPA 실전이 RESERVE 상태로는 안 보임
2. Kafka UI(http://localhost:8989) → Topics → `order.enrollment.completion` → **Produce Message** (주문 서비스가 보낸 메시지를 흉내내는 셈)
   ```json
...
4. 학생A 수강 목록 다시 조회 → JPA 실전이 ACTIVE 로 추가됨
5. 같은 메시지 한 번 더 발행 → `skipped(idempotent)=1` 로그 (멱등성 검증)
...
→ Kafka 가 아직 안 떴거나 헬스체크 실패. KRaft 모드는 첫 부팅 시 메타데이터 디렉토리 초기화로 30초 정도 걸림. `docker compose logs kafka` 로 `Kafka Server started` 확인 후 앱 재기동.

### `[Kafka] Received` 로그가 안 찍힘

→ Consumer 그룹의 `auto-offset-reset` 이 `latest` 라 컨슈머가 등록되기 전에 발행한 메시지는 못 받음. 앱 기동 후에 메시지 발행할 것. 이미 발행한 메시지를 처리하려면 `kafka-consumer-groups.sh --reset-offsets --to-earliest`.
...
→ `dev` 프로파일로 떴는데 Config Server 가 안 켜져있을 때 발생. 로컬에서는 `SPRING_PROFILES_ACTIVE=local` 확인 (또는 미지정). `bootstrap.yaml` 이 local 일 땐 Config Server 비활성화.
...
└── src/main/
    ...
    │   ├── application/        # 응용 계층 (Use case 서비스 / DTO)
    ...
    └── resources/
        ...
        └── bootstrap.yaml      # Config Server 위치 (dev only)
```
